...
В этом разделе вы можете настроить работу с ботом, обновить информацию о себе, а также изменить цели.
'''
...
def daily_change_text(user):
    if user:
        return \
f'''
<b>Текущие значения (за сутки)</b>

Количество приемов пищи: {user.daily_eating_norm}
Норма калорий: {user.daily_kcal_norm}
<i>Напоминаем, эти параметры устанавливаются автоматически исходя из остальных ваших данных</i>

<b>Выберите что бы вы хотели изменить:</b>
'''
    else:
        return \
f'''
<i>Текущие значения не установлены, рекомендуем сначала пройти регистрацию в приложении</i>

<b>Выберите что бы вы хотели установить:</b>
'''
...
Выберите часовой пояс. Благодаря этому бот будет лучше подбирать рекомендации по питанию исходя из времени суток.

Ваш часовой пояс: GMT-{'+' if x > 0 else ''}{x}'''

utc_edit_end = 'Часовой пояс изменен'

daily_goal_text = 'Отправьте количество ккал, которое хотите установить как дневную норму'
daily_goal_end = 'Данные о суточной норме калорий изменены'
